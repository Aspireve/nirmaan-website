  ...
  const renderPageNumbers = () => {
    ...
    for (let i = startPage; i <= endPage; i++) {
      pageNumbers.push(
        <div
          key={i}
          className={`px-4 ${currentPage === i ? 'text-gray-500 cursor-not-allowed' : ''}`}
          onClick={() => paginate(i)}
        >
          {i}
        </div>
  ...
  return (
    ...
    <div>
      ...
      <div className="flex md:justify-between justify-center text-white md:w-[48rem] xl:w-[65rem] w-96 ">
        <div className="md:flex  mt-5 text-2xl hidden">
            See More....
        </div>
        <div className="flex mt-5 cursor-pointer ">
        <div
                className={`px-4 ${currentPage === 1 ? 'text-gray-500 cursor-not-allowed' : ''}`}
                onClick={() => paginate(1)}
              >
                &lt;&lt;
              </div>
              ...
              {renderPageNumbers()}
              <div
                className={`px-4 ${currentPage === totalPages ? 'text-gray-500 cursor-not-allowed' : ''}`}
                onClick={() => paginate(currentPage + 1)}
              >
                &gt;
              </div>
